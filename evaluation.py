...
# Extract from following model_eval_multitask function
def model_eval_para(dataloader, model, device, cross=False):
    model.eval()  # switch to eval model, will turn off randomness like dropout
    with torch.no_grad():
        para_y_true = []
        para_y_pred = []
        ...
        for step, batch in enumerate(tqdm(dataloader, desc=f'eval', disable=TQDM_DISABLE)):
            if cross:
                (token_ids, token_type_ids, attention_mask,
                 b_labels, b_sent_ids) = (batch['token_ids'], batch['token_type_ids'],
...
# Extract from following model_eval_multitask function
def model_eval_sts(dataloader, model, device, cross=False):
    model.eval()  # switch to eval model, will turn off randomness like dropout
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(tqdm(dataloader, desc=f'eval', disable=TQDM_DISABLE)):
            if cross:
                ...
                logits = model.predict_similarity(token_ids, attention_mask, token_type_ids=token_type_ids)
            else:
                (b_ids1, b_mask1,
                b_ids2, b_mask2,
                b_labels, b_sent_ids) = (batch['token_ids_1'], batch['attention_mask_1'],
...
def model_eval_multitask(sentiment_dataloader,
                         ...
                         sts_dataloader,
                         models, device, cross=False):
    sst_model, para_model, sts_model = models['sst'], models['para'], models['sts']
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(tqdm(paraphrase_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
            if cross:
                ...
                logits = para_model.predict_paraphrase(token_ids, attention_mask, token_type_ids=token_type_ids)
            else:
                (b_ids1, b_mask1,
                b_ids2, b_mask2,
                b_labels, b_sent_ids) = (batch['token_ids_1'], batch['attention_mask_1'],
        ...
        for step, batch in enumerate(tqdm(sts_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
            if cross:
                ...
                logits = sts_model.predict_similarity(token_ids, attention_mask, token_type_ids=token_type_ids)
            else:
                (b_ids1, b_mask1,
                b_ids2, b_mask2,
                b_labels, b_sent_ids) = (batch['token_ids_1'], batch['attention_mask_1'],
        ...
        print(f'Semantic Textual Similarity correlation: {sts_corr:.4f}')
...
def model_eval_test_multitask(sentiment_dataloader,
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(tqdm(paraphrase_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
            if cross:
                (token_ids, token_type_ids, attention_mask,
                 b_sent_ids) = (batch['token_ids'], batch['token_type_ids'],
                            batch['attention_mask'], batch['sent_ids'])
                token_ids = token_ids.to(device)
                token_type_ids = token_type_ids.to(device)
                attention_mask = attention_mask.to(device)
...
def test_model_multitask(args, models, device):
        ...
        para_test_data = SentencePairTestDataset(para_test_data, args, cross=args.cross)
        ...
        sts_test_data = SentencePairTestDataset(sts_test_data, args, cross=args.cross)
        ...
        dev_paraphrase_accuracy, dev_para_y_pred, dev_para_sent_ids, \
            ...
            dev_sts_y_pred, dev_sts_sent_ids = model_eval_multitask(sst_dev_dataloader,
                                                                    para_dev_dataloader,
                                                                    sts_dev_dataloader, models, device, cross=args.cross)

        test_para_y_pred, test_para_sent_ids, test_sst_y_pred, \
            test_sst_sent_ids, test_sts_y_pred, test_sts_sent_ids = \
                model_eval_test_multitask(sst_test_dataloader,
                                          para_test_dataloader,
                                          sts_test_dataloader, models, device, cross=args.cross)

        with open(args.sst_dev_out, "w+") as f:
            print(f"dev sentiment acc :: {dev_sentiment_accuracy :.4f}")
        ...
        with open(args.para_dev_out, "w+") as f:
            print(f"dev paraphrase acc :: {dev_paraphrase_accuracy :.4f}")
        ...
        with open(args.sts_dev_out, "w+") as f:
            print(f"dev sts corr :: {dev_sts_corr :.4f}")
