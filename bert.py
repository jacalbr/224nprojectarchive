...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    # batch size, num_head, Sequence Length, Head dim
    N, H, T, Hdim = key.shape

    scale = 1/torch.sqrt(torch.tensor(Hdim))
    scale = scale.type_as(query)

    # adjust dimension for (N, H, Hdim, T) for operation on last to dim
    Q = query
    K = key.permute(0, 1, 3, 2)
    V = value

    # attention (N, H, T, Hdim) @ (N, H, Hdim, T) -> (N, H, T, T)
    score = torch.matmul(Q, K)*scale
    # apply mask
    score += attention_mask
    A = self.dropout(F.softmax(score, dim=-1))

    # output (N, H, T, T)@(N, H, T, Hdim) -> (N, H, T, Hdim)
    Y = torch.matmul(A, V)
    out = Y.permute(0, 2, 1, 3).contiguous().view(N, T, H*Hdim)
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    interm_x = self.interm_dense(x)
    interm_x = self.interm_af(interm_x)
    out = self.add_norm(x, interm_x, self.out_dense, self.out_dropout, self.out_layer_norm)
    return out 
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids, tk_type_ids = None):
    ...
    out_embeds = inputs_embeds + pos_embeds + tk_type_embeds
    out_embeds = self.embed_layer_norm(out_embeds)
    out_embeds = self.embed_dropout(out_embeds)
  ...
  def forward(self, input_ids, attention_mask, tk_type_ids=None):
    """
    input_ids: [batch_size, seq_len], seq_len is the max length of the batch
    attention_mask: same size as input_ids, 1 represents non-padding tokens, 0 represents padding tokens
    """
    # get the embedding for each input token
    embedding_output = self.embed(input_ids=input_ids, tk_type_ids=tk_type_ids)
